...
func generateChaosFile(gen *protogen.Plugin, file *protogen.File) {
	...
	for _, method := range service.Methods {
		...
		g.P()
		g.P(`		http.Post(fmt.Sprintf("http://%s/api/chaos?id=%s", config.AdminPanelURL, chaosID), "application/json", nil)`)
		g.P()
		...
		g.P(`		http.Post(fmt.Sprintf("http://%s/api/chaos?id=%s", config.AdminPanelURL, chaosID), "application/json", bytes.NewBuffer(jsonString))`)
	...
	g.P(`	response, err := http.Get(fmt.Sprintf("http://%s/api/service/staging_address?name=%s", c.AdminPanelURL, c.ServiceName))`)
	g.P("	if err != nil {")
	g.P(`		return err`)
	g.P("	}")
	...
	g.P(`		AdminPanelURL: "127.0.0.1:9033",`)
