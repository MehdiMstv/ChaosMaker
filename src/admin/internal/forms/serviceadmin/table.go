...
func (g *ServiceGenerator) GetTable(ctx *context.Context) table.Table {
	...
	info.AddField("Name", "name", db.Text).
		FieldFilterable().FieldSortable()
	info.AddField("Address", "address", db.Text).FieldSortable()
	info.AddField("Staging Address", "staging_address", db.Text).FieldSortable()

	info.SetTable("services").SetTitle("Service").SetDescription("Service")

	formList := serviceTable.GetForm()
	formList.AddField("Service Name", "name", db.Text, form.Text).FieldMust()
	...
	formList.SetTable("services").SetTitle("Service").SetDescription("Service")
	return serviceTable
}
